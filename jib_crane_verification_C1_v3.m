...
%  Project ID 40467361  |  MATLAB R2026a  |  C1_v3 (Q235 + 980 mm front line)
%
%  REVISION v2 -> v3 (material + front-overhang correction):
%    1. MATERIAL. The crane material is Q235 (GB/T 700 carbon structural
%       steel), not S275. fy lowered 275 -> 235 MPa, fu 430 -> 370 MPa. All
%       sections are 6 mm wall (< 16 mm), so the full 235 MPa applies with no
%       thickness de-rating. Member allowable 235/1.5 = 156.7 MPa (was 183.3).
%       Q235 is the GB/T 700 equivalent of ASTM A36 / EN S235JR.
%    2. FRONT OVERHANG. base.L_fwd corrected 1.200 -> 0.980 m. The base
%       top-view chain (slew centre ~1076 mm behind the front edge) and the
%       warning plate (1400 - 420 = 980 mm) agree, so the front tipping line
%       sits 980 mm ahead of the column. This is the wheels-only companion to
%       the C2 deployed code and matches the Combined / Re-Rating reports.
%    3. Proof-load-test references removed; Test 9 reframed as a theoretical
%       elastic / plastic capacity check (physical testing is done separately
%       with the approval officer).
%
...
%     6. Stability / overturning at every load-chart point
...
% at the GB/T 700 minimum 370 MPa. Q235 = ASTM A36 / EN S235JR equivalent.
% Replaces the S275 assumption of v2 (fy was 275, fu was 430).
...
% Mill sensitivity: Q235 mill yield commonly runs ~255-275 MPa; nominal 235
% used for all verdicts (conservative).
...
%   *** RESOLVED (was VERIFY in v2) ***  The warning plate's 420 mm front
...
%   1400 - 420 = 980 mm ahead of the slew axis. This matches the base
%   top-view chain (slew centre ~1076 mm behind the front edge), so
%   base.L_fwd = 0.980 m is adopted (the earlier 1.200 m was a
%   non-conservative place-holder). e_cg still to be confirmed by weighing.
base.L_fwd   = 0.980;       % m, column centre -> front wheel (tipping line) RESOLVED v3
