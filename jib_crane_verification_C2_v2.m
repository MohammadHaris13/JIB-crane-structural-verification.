...
%  Project ID 40467361  |  MATLAB R2026a  |  C2_v2 (legs-deployed track)
%
%  REVISION v1 -> v2 (geometry resolved against full BASE drawing):
%    The forward tipping line base.L_fwd is no longer an open VERIFY item.
%    The BASE TOP VIEW dimension chain (2360 = front edge to slew centre
%    ~1076 mm) and the manufacturer warning plate (shortest chart reach
%    1400 mm shown with a 420 mm front dimension => 1400 - 420 = 980 mm of
%    forward overhang from the slew axis to the front tipping line) both
%    give the SAME number. base.L_fwd is therefore set to 0.980 m (was the
%    optimistic 1.200 m place-holder). This is the governing change: the
%    forward stability margin at the rated chart drops accordingly and the
%    crane no longer clears the FoS target at the marked loads - which is
%    exactly what the separate re-rating (Report Rev R1) corrects.
%
