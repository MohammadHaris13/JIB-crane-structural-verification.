...
% Slewing-centre position, traced through the BASE drawing dimension chain:
%   front overhang 220 + offset 20 + 914 + half slewing housing (~46)
%   = 1200 mm from front edge of base (and 1160 mm from rear edge).
% The Assembly front view shows the 2360 dimension as the WHEELBASE (wheels
% at the four corners of the base footprint), so column-to-wheel distances
% are read directly from the dimension chain above:
base.L_fwd   = 1.200;       % m, column centre -> front wheel (tipping line)
base.L_rwd   = 1.160;       % m, column centre -> rear wheel
base.wheelD  = 0.150;       % m, 4 x Phi150 wheels (warning plate "4-Φ150")
base.h_base  = 0.190;       % m, base frame depth (BASE front view "190")

% --- Hydraulic cylinders (from Arm Section dwg, callouts on front view) ---
% Naming: "Type 80" / "Type 50" are the bore in mm; "core diameter" is the rod.
cyl_lift = struct('bore',0.080,'rod',0.050,'stroke',0.700, ...
                  'tag','Type 80 hyd. cyl - bore 80mm, rod 50mm, stroke 700mm');
cyl_luff = struct('bore',0.050,'rod',0.030,'stroke',0.775, ...
                  'tag','Type 50 hyd. cyl - bore 50mm, rod 30mm, stroke 775mm');
cyl_anchor_col  = 0.30;     % m, cylinder bottom pin above column base  (ESTIMATE)
cyl_anchor_boom = 0.40;     % m, cylinder top pin along boom from pivot (ESTIMATE)

% --- Pins (from Arm Section detail-D and Base detail-12) ---
pin.d_pivot = 0.030;        % m, boom pivot pin Ø30
pin.d_cyl   = 0.020;        % m, cylinder pin
pin.d_hook  = 0.020;        % m, hook eye pin (Detail-12, Ø20)
pin.t_plate = 0.0075;       % m, lug plate thickness (T7.5)

% --- Slewing flange (Base Detail A-A) ---
slew.n_bolt = 5;            % 5 x M16
slew.PCD    = 0.170;        % m
slew.d_bolt = 0.016;        % nominal
...
% Hydraulic cylinder force: solve for the lift cylinder reaction.
% Take moments about boom pivot.  Cylinder line of action is roughly the
% line from cylinder bottom anchor (on column) to top anchor (on boom).
...
fprintf('  Cylinder force        F_cyl   = %.2f kN\n', F_cyl/1e3);
