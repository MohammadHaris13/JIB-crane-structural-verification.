...
%  Project ID 40467361  |  MATLAB R2026a  |  C2_v3 (Q235 + updated drawing)
%
%  REVISION v2 -> v3 (material change + general-arrangement drawing update):
%    1. MATERIAL. The crane material is Q235 (Chinese carbon structural
%       steel), not S275. fy is therefore lowered from 275 MPa to the Q235
%       nominal yield 235 MPa, and fu from 430 MPa to the Q235 minimum
%       ultimate 370 MPa. All sections are 6 mm wall (< 16 mm), so the full
%       235 MPa applies with no thickness de-rating. Q235 is the GB/T 700
%       equivalent of ASTM A36 / S235JR. This is a ~15% drop in yield and
%       lowers every stress-based FoS (member allowable now 235/1.5 =
%       156.7 MPa, down from 183.3 MPa). Stability ratios are unaffected by
%       material (they are moment balances), so forward/sideways tipping
%       numbers do not move with the grade.
%    2. DEPLOYED TRACK. The updated general-arrangement plan view dimensions
%       the deployed lateral track foot-to-foot at 2500 mm (the "2500"
%       dimension), replacing the 2376 mm previously derived from the bare
%       leg-arm projection. Half-track is therefore 1250 mm, which improves
%       sideways stability slightly. The "1100" dimension is the inner frame
%       width. The 2360 mm wheelbase (side view) is unchanged, so the
%       980 mm front overhang and forward-stability result are unchanged.
%
%  REVISION v1 -> v2 (geometry resolved against full BASE drawing):
%    The forward tipping line base.L_fwd was resolved from the BASE TOP VIEW
%    dimension chain (2360 = front edge to slew centre ~1076 mm) and the
%    manufacturer warning plate (1400 - 420 = 980 mm forward overhang), both
%    giving 980 mm. base.L_fwd = 0.980 m (was the 1.200 m place-holder).
%
...
%     9. Elastic and plastic capacity of the column (theoretical)
...
%       General notes: drawing lists steel S275, but the client has
%       confirmed the crane material is Q235 (used here); plates ASTM A36 or
%       higher, bolts Gr 8.8,
...
%   3. Counterweight / pump-unit CG -> e_cg (still by weighing). The
%      longitudinal wheel/tipping line base.L_fwd is now RESOLVED: the BASE
%      TOP VIEW chain (front edge -> slew centre ~1076 mm) and the warning
%      plate (1400 - 420 = 980 mm forward overhang) agree, so base.L_fwd =
%      0.980 m is used (no longer the 1.200 m place-holder). e_cg and the
%      rear wheel line base.L_rwd remain to be confirmed by weighing.
% =====================================================================
...
% --- Material : Q235 carbon structural steel (GB/T 700) ---
% Q235 nominal yield 235 MPa for thickness <= 16 mm; all sections are 6 mm
% wall, so the full 235 MPa applies (no thickness de-rating). Ultimate taken
% at the GB/T 700 minimum 370 MPa. Q235 is the equivalent of ASTM A36 /
% EN S235JR. Replaces the S275 assumption of v2 (fy was 275, fu was 430).
fy = 235e6;                 % Q235 steel, nominal yield (t <= 16 mm)
fu = 370e6;                 % Q235 minimum ultimate (GB/T 700)
...
% Mill sensitivity: Q235 mill yield commonly runs ~255-275 MPa; the nominal
% 235 MPa is used for all verdicts (conservative).
fy_actual    = 265e6;       % typical Q235 mill yield (sensitivity vs nominal 235)
...
% ~1076 mm back from the front edge (1990 - 914 off the dimension chain),
% with the front wheel close to the front frame line.
%   *** RESOLVED (was VERIFY in v1) ***  The warning plate's 420 mm front
%   dimension is the load at the shortest chart reach (1400 mm) sitting
%   ~420 mm beyond the front tipping line, i.e. the front tipping line is
%   1400 - 420 = 980 mm ahead of the slew axis. This matches the top-view
%   chain, so base.L_fwd = 0.980 m is adopted (the earlier 1.200 m was a
%   non-conservative place-holder). e_cg still to be confirmed by weighing.
base.L_fwd   = 0.980;       % m, slew axis -> front wheel (tipping line) RESOLVED v2
base.L_rwd   = 1.160;       % m, slew axis -> rear wheel                 VERIFY (weighing)
...
legs.L_fwd_dep  = base.L_fwd + legs.arm*cos(deg2rad(legs.splay_deg)); % = base.L_fwd (0.980 m)
% Deployed lateral track taken DIRECTLY from the updated general-arrangement
% plan view, which dimensions the foot-to-foot width at 2500 mm (the "2500").
% This supersedes the 2376 mm previously derived from the bare 750 mm leg-arm
% projection (0.438 + 0.750 half-track). The "1100" plan dimension is the
% inner frame width. Half-track is therefore 1250 mm.
legs.track      = 2.500;            % m, deployed foot-to-foot track (PLAN "2500")
legs.track_half = legs.track/2;     % m, = 1.250 m
...
%% ---------------------------------------------------------------------
%  TEST 9.  ELASTIC AND PLASTIC CAPACITY OF THE COLUMN (theoretical)
%  ---------------------------------------------------------------------
%  The elastic working-stress checks above put the column at FoS ~ 1.0
%  (first-fibre yield) at the marked loads. First yield is not collapse.
%  The working-stress check is a serviceability-style criterion (first yield
%  with a 1.5 target), whereas the section's true limit-state capacity is
%  governed by the plastic moment Mp = Zx*fy, which sits above first yield.
%  RHS sections have a shape factor Zx/Sx ~ 1.2-1.25, so there is real
%  reserve between first yield and a plastic hinge. This block reports that
%  reserve as a theoretical capacity statement only. Physical load testing
%  is carried out separately with the approval officer and is not modelled
%  here.
fprintf('=== TEST 9 : ELASTIC AND PLASTIC CAPACITY OF THE COLUMN ===========\n');
...
fprintf('     plastic Mp (nom Q235)  = %6.2f kN.m  -> reserve over yield %+.0f%%  %s\n', ...
        Mp_col_nom/1e3, (Mp_col_nom/My_col_nom-1)*100, proof_txt(M_applied<=Mp_col_nom));
fprintf('     plastic Mp (act %3.0f)   = %6.2f kN.m  -> reserve over applied %+.0f%%  %s\n', ...
        fy_actual/1e6, Mp_col_act/1e3, (Mp_col_act/M_applied-1)*100, proof_txt(M_applied<=Mp_col_act));

% Other members against yield at the marked load (should remain elastic)
fprintf('  Other members @1.25xSWL (vs fy=%.0f MPa):\n', fy/1e6);
fprintf('     boom outer  sigma = %3.0f MPa  %s\n', sig_outer/1e6, elastic_txt(sig_outer<fy));
fprintf('     outrigger   sigma = %3.0f MPa  %s\n', sig_base/1e6,  elastic_txt(sig_base<fy));
fprintf('  CONCLUSION: the column reaches first-fibre yield near the marked\n');
fprintf('     load, but its plastic moment lies above the applied moment, so\n');
fprintf('     the section retains reserve beyond first yield. The elastic\n');
fprintf('     FoS<1.5 is a conservative working-stress result, not a\n');
fprintf('     prediction of collapse.\n\n');
...
fprintf('     the %.0f mm drawn track is compared against that requirement; the\n', legs.track*1e3);
fprintf('     as-built deployed width should be confirmed on the machine.\n');
...
if save_outputs
    ...
    save(fullfile(out_dir,'workspace.mat'), ...
         ...
         'My_col_nom','Mp_col_nom','My_col_act','Mp_col_act','M_applied', ...
         'fy_actual','F_cyl','p_cyl_req', ...
...
function s = proof_txt(ok)
    if ok, s = 'within Mp'; else, s = '*** EXCEEDS Mp ***'; end
end
